...
test("T4 (retrofit directivo): el aviso dice cuándo NO compensa pedir la deuda según el escenario base, y sigue exigiendo verificación profesional real", () => {
  const result = Simulator.simulateLeverage({
    barrierResult: readyBarrier(),
    newDebtAmount: 10000,
    newDebtAnnualRatePercent: 5,
  ...
  assert.match(result.warning, /verifica cualquier decisión real con un profesional/i);
});
