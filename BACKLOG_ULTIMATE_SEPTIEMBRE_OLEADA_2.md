...
| Orden | ID | Tarea | Origen | Esfuerzo | Beneficio | Nota |
|---|---|---|---|---|---|---|
| 46 | ✅ ESX1 | Monte Carlo de trayectorias | Escenarios 2 | L | Alto | Hecho — `monteCarloSimulation()` amplía `prudentSimulation()` (A8-3): cientos de trayectorias (no miles, decisión explícita del usuario tras validar coste contra `OPT-5`), en el hilo principal. Cada mes muestrea el triángulo(P10, P50, P90) ya calibrado por `prudentSimulation()`; da probabilidad de ruptura y banda P10/P50/P90 de la caja mínima. Tope duro de 500 trayectorias |
...
| 48 | ✅ ESX3 | Escenario inverso: «¿qué tendría que cambiar?» | Escenarios 2 | L | Alto | Hecho — `inverseScenario()` (`canonical-e13-scenarios.js`): busca el punto de cruce EXACTO por bisección sobre `simulate()` (A8-1) para el horizonte completo, extendiendo la extrapolación lineal de un solo paso de `sensitivity()` (A8-6) con la precisión algebraica de `verdictSensitivity()` (PV6). Un caso ya roto no busca cruce hacia delante; un eje sin cruce en el rango explorado lo dice con su nota. Al construirla se encontró y corrigió un bug real en `simulate()` (`|| 1` trataba un factor 0 como "sin dato") |
| 49 | ✅ IVX3 | Activos alternativos | Inversión 2 | L | Medio | Hecho — tipo propio `"alternativo"` en `canonical-assets.js` (A14-1), con formulario de alta propio (categoría, importe invertido) igual que el alta de créditos. `alternativeAssetReturn()` deriva la rentabilidad del valor actual y el importe invertido ya declarados, nunca un tercer porcentaje pedido aparte; sin importe invertido conocido, `calculable: false` |
| 50 | ⚪ FCX2 | Simulador de cambio de residencia fiscal | Fiscalidad 2 | L | Bajo-Medio, oportunista | Evaluada y descartada el 5-sep: el usuario confirma que la residencia fiscal será siempre España. Sin esa hipótesis real no hay escenario que simular — construirlo habría sido invertir esfuerzo L en una pregunta que este hogar no se hace |
