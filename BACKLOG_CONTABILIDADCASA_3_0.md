...
| 2 — este trimestre | ~~`PER-1`→`PER-2`→`PER-3`~~ cerradas (22/09/2026, sesión 221); queda `PER-4` (selector de UI); `I5` (heredada, sin bloqueo); `ARQ-1`, `ARQ-2` | El modelo de periodo es la única pieza de este documento con caducidad real — cuantos más motores existan antes de generalizarlo, más caro migrarlos después. El módulo (`canonical-period.js`) y su migración ya están construidos; falta llevarlo a la UI (`PER-4`). `I5` ya era la siguiente tarea acordada con el hogar (2.0 §6) — no tiene sentido reordenarla por detrás de trabajo nuevo |
