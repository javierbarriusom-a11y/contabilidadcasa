...
**Estado (sesión 188, 13 de septiembre de 2026): `GOB14` construida.** Bloque 1 completo (`VER-4`,
...
de `GOB12`; `GOB12` (sesión 186); `GOB13` (sesión 187); `GOB14` (sesión 188). Quedan 4 tareas
accionables: `GOB16`, `GOB18` del Bloque 7, y las dos apuestas grandes (`GOB15`, `GOB19`, reservadas
a sesión propia cada una).
...
| 6 | ✅ `PVC14` | Ensemble ponderado y visible entre histórico, manual y Monte Carlo | `PA-5` | L | Alto | **Hecho (sesión 178).** Diagnóstico corrigió el planteamiento: el "manual" no existía (las llamadas pasaban `{min:-500,base:0,max:500}` hardcodeado, nunca declarado), y Monte Carlo no es un tercer método pesable — ya se construye encima del triángulo de `prudentSimulation()`. Nueva `ensembleForecastRange()` (`canonical-e13-scenarios.js`) mezcla histórico y un manual DECLARADO por el hogar con el peso ya confirmado en sesión 171 (ajustable, no fijo); Monte Carlo hereda el resultado sin cambio de código. Los dos triángulos de origen se muestran siempre por separado, nunca solo la cifra mezclada (cumple la advertencia de este mismo punto y del §11). De paso se corrigió un `TypeError` real ya existente (`pvc2CategoryConfidenceShare` asumía `prudent.percentiles` siempre no nulo). Tests: `tests/pvc14-ensemble-historico-manual.test.cjs` (21 tests). |
...
| 8 | ✅ `PVC16` | Marcar eventos no recurrentes para que no contaminen el sesgo aprendido | `PA-7` | M | Medio | **Hecho (sesión 174).** Nuevo `record.nonRecurring === true` (declarado por el hogar, con motivo obligatorio, nunca inferido) excluido de `usable` tanto en `learnFromHistory()` como en `detectStructuralChange()` (`canonical-forecast.js`) — el mismo mes marcado no contamina ni el sesgo aprendido ni la detección de persistencia de `PVC3`. Persistencia declarada en `pvc16-non-recurring-months` (motivo + fecha), propagada por `reconciledMonthlyNetHistory()`. Se muestra en Análisis de previsión, justo debajo del backtesting de PVX1 (que además anota inline qué meses quedan excluidos y por qué), con selector de mes + motivo y lista de meses marcados con botón para desmarcar. Tests: `tests/pvc16-eventos-no-recurrentes.test.cjs` (11 tests). |
| 9 | ✅ `PVC17` | Índice único de "salud predictiva", con tendencia | `PA-8` | S | Medio | **Hecho (sesión 174).** Nueva `predictiveHealthIndex()` (`canonical-e16-monitoring.js`) agrega el `meanAbsoluteError` global que `predictionQuality()` ya calculaba (ponderado por muestra, no por categoría a partes iguales) en una única cifra — sin inventar una escala 0-100 (exigiría un umbral de "error aceptable" que el hogar no ha declarado), el índice ES el MAE en euros. La tendencia compara contra la última medición guardada en el mismo cierre de mes que ya recalibra PV3 (`pvc17-health-snapshot`); sin medición previa, "sin-historial" explícito, nunca "estable" por defecto. Se muestra en Análisis de previsión, con el desglose por categoría que antes solo existía disperso. Tests: `tests/pvc17-salud-predictiva.test.cjs` (15 tests). |
| 10 | ✅ `PVC18` | Etiquetar la causa de cada cambio de previsión: dato nuevo, supuesto editado o modelo recalibrado | `PA-9` | S-M | Medio | **Hecho (sesión 174).** Alcance reducido confirmado: sobre la comparación que `PVC6` (Oleada 3) ya hace con `diffAssumptionSnapshots()`, nueva `pvc18ChangeCauses()` (`app.js`) etiqueta hasta tres causas no excluyentes leyendo piezas que ya existían — "supuesto editado" es lo que el propio diff detecta; "dato nuevo" y "modelo recalibrado" se leen del diario de PV5 (ya con marca de tiempo) filtrado a lo posterior al cierre comparado, siendo "modelo recalibrado" el subconjunto que además cruzó el umbral de confianza alta de PV1 (`pv1AutoAdjustTransitionNote`, reutilizada tal cual). Ningún motor de comparación nuevo. Tests: `tests/pvc18-causa-cambio-prevision.test.cjs` (13 tests). |
| 11 | ✅ `PVC19` | El cono de incertidumbre debe verse como un cono, no como una banda de grosor fijo | `PA-10` | S | Medio | **Hecho (sesión 174).** Corrección de renderizado únicamente (`pv4ConfidenceBandHtml()`, `app.js`): las columnas de barras sueltas por mes (sin conexión visual entre ellas) se sustituyen por un polígono SVG continuo entre el límite bajo y alto de cada mes — la forma de cono en sí — más una línea central, usando exactamente los mismos `low`/`high`/`center` que ya calculaba `confidenceBands()`, sin tocar su cálculo. Tests: `tests/pvc19-cono-incertidumbre.test.cjs` (7 tests) más la actualización del test existente de PV4. |
...
| 31 | ✅ `DEB12` | El precio de esperar (`waitingOptionValue`) como serie temporal, no cifra congelada | `DE-4` | M | Medio | **Hecho (sesión 182).** `scenarioSettings.deb3TrackedWait` guarda el importe/TIN/meses declarados y la fecha en que se declaró la espera (`evaluatedAt`), mismo patrón de "comparación trackeada" que `DEB1` (`ap1TrackedComparison`). Nueva `deb12WaitingCostSoFarHtml()` (`app.js`) recalcula `waitingOptionValue()` (sin motor nuevo) con los meses REALES transcurridos desde entonces (nunca más que los declarados), así el coste avanza solo con el calendario en vez de quedarse congelado en la cifra del día que se comparó. Tests: `tests/deb12-precio-esperar-serie-temporal.test.cjs`. |
...
| 33 | ✅ `DEB14` | Alerta de "llevas N meses sin comparar tu hipoteca contra una oferta de mercado registrada" | `DE-6` | S | Medio | **Hecho (sesión 184).** Alcance reducido: distinta de `DEB4` ("tu cálculo cambió"). Nueva `deb14MarketCheckAlertHtml()` (`app.js`) cruza los contratos declarados con `type` conteniendo "hipoteca" (`debtContractSourceRows()`) con el registro de ofertas externas de Deuda › Ruta (`e14bWorkspace().offers`, `normalizeOffer`) — meses reales desde la oferta más reciente, umbral declarado (`#deb14MaxMonthsWithoutOffer`), sin motor nuevo. Tests: `tests/deb14-cuanto-hace-que-miraste-mercado.test.cjs`. |
