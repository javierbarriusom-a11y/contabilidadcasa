...
| 3 — este semestre | ~~`FLU-2`~~ cerrada (23/09/2026, sesión 225); orden confirmado con el hogar para el resto: `NAV-2` → `FIN-1` → `NAV-1`/`FLU-1`/`NAV-4` → `ARQ-3` → `ARQ-4` (continúa `T14`)/`T9` (heredada) | Impacto directo en uso diario y deuda técnica, sin requerir motores nuevos ni depender de telemetría |
