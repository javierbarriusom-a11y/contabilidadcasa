(function (root) {
  ...
  // PVC1 (Oleada 3, Bloque 2): VER-1 confirmó que el forecast ya se repropaga a metas (E15) y riesgo
  // (E16) sin abrir estas pantallas — recomputeModelIfNeeded() recalcula por firma en cada render()
  // global. Lo único que faltaba era hacerlo visible: esta línea muestra cuándo ocurrió de verdad el
  // último recálculo, para que el hogar no tenga que confiar a ciegas en que los números están al día.
  function recomputedAgoHtml() {
    const iso = bridge()?.lastModelRecomputeAt?.();
    if (!iso) return "";
    const minutes = Math.max(0, Math.floor((Date.now() - new Date(iso).getTime()) / 60000));
    const label = minutes < 1 ? "hace un momento"
