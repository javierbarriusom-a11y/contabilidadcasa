...
| ID | Tarea | Esfuerzo | Beneficio | Nota |
|---|---|---|---|---|
| `PER-1` | Nuevo módulo `canonical-period.js`: `periodKey`/`periodRange`/`periodLabel`/`adjacentPeriod` para `month`\|`quarter`\|`semester`\|`year`, con tests propios | M | Alto | Generaliza lo que `CanonicalBudgetSchema` ya resolvió para trimestre/año (`BUD-3`) a un solo módulo consumible por cualquier motor. Sin motor de negocio propio — solo cálculo de rangos de fecha y claves, mismo shape que `quarterRange`/`annualRange` ya en producción. |
| `PER-2` | Migrar `CanonicalBudgetSchema.currentQuarterKey`/`quarterRange`/`annualRange` a delegar en `canonical-period.js` | S | Medio | Refactor interno, mantiene la API pública intacta — no debe romper ninguno de los tests de `BUD-3` ni de `bud3-presupuesto-anual-trimestral.test.cjs`. Verifica que la generalización es real antes de construir semestre encima. |
| `PER-3` | Añadir `semester` (`"2026-S1"`/`"2026-S2"`, enero-junio/julio-diciembre) como cuarta cadencia en `canonical-period.js` | S | Alto | Es la única cadencia que hoy no existe en ningún sitio del código (confirmado por grep completo del repositorio: «semestral» solo aparece en `PROJECT_STATE.md`, nunca en código). Trivial una vez `PER-1`/`PER-2` están construidos. |
| `PER-4` | Selector de periodo único de UI (mes/trimestre/semestre/año) reutilizado en Presupuesto, Análisis, Previsión y salud financiera; informe semestral de familia extendiendo `gob14ReportBodyHtml()` (mismo patrón que `P12`, mensual) | M | Alto | Sustituye el toggle propio de `views/presupuesto-mes.js` por el componente único; añade la opción que hoy falta (semestre) al resto de pantallas que ya usan alguna cadencia. Preferencia de cadencia persistida por pantalla, no forzada globalmente (alguien puede querer Presupuesto en trimestral y Patrimonio en anual). |

### 2.2 Arquitectura y deuda técnica (`ARQ`)

| ID | Tarea | Esfuerzo | Beneficio | Nota |
|---|---|---|---|---|
| `ARQ-1` | TypeScript incremental (`checkJs: true` + JSDoc, sin migrar sintaxis) sobre los `canonical-*.js` | M | Alto | No cambia la sintaxis en tiempo de ejecución — compatible con `vm.Script`, a diferencia de los módulos ES. Es la única forma real de detectar en tiempo de escritura los cambios de forma de datos entre los 64 motores que hoy solo atrapan los tests. |
| `ARQ-2` | ESLint con reglas mínimas (variables no usadas, igualdad estricta, complejidad ciclomática) como gate de CI | S | Medio | Hoy no hay `.eslintrc` ni ningún linter — nada impide que la complejidad de `app.js` (38.340 líneas) siga creciendo sin control. |
...
|---|---|---|---|
| `I5` | Rescate de pensiones (reducción por antigüedad + modalidad renta) | Ninguna — siguiente en cola según el propio hogar (2.0 §6, sesión 218) | 2.0 §2 |
