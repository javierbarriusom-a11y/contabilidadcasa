...
| `ARQ-3` | ~~Detección sistemática de código muerto/huérfano...~~ — **cerrada el 23 de septiembre de 2026 (sesión 231)** | M | Medio | Auditoría completa de los 65 `canonical-*.js`: 59/65 consumidos de verdad; 6/65 huérfanos, ninguno nuevo como hallazgo de fondo — el caso ya conocido (`canonical-e9-assistant.js`, Copiloto/IA) resultó ser la punta de un grupo de 5 motores más de la misma épica E9, bloqueados por la misma condición externa `A5-1` ya documentada. `tests/arq3-canonical-sin-consumidor-ui.test.cjs` convierte esa verificación en invariante (falla ante un huérfano nuevo o una excepción ya obsoleta). Las 10 pantallas «legacy» no necesitaron auditoría nueva: `LABORATORIO_CATALOG`/`laboratorioWriteGuard()` (`app.js`, regla `L-5`) ya es exactamente esa vara, pantalla por pantalla — "legacy" en el menú es posición de navegación, no estado funcional (4 de las 10 siguen siendo puerta de escritura real fuera de Laboratorio). Detalle completo en el cierre de sesión 231 de `PROJECT_STATE.md`. |
| `ARQ-4` | **⏸ En pausa desde el 24 de septiembre de 2026 (sesión 238), decisión del hogar.** Continuación de `T14` (2.0 §4) con el patrón ya decidido — más incrementos de `views/*.js` con carga diferida sobre el resto de `app.js` | L | Alto | **No** introduce `import`/`export` ni `import()` dinámico (conflicto confirmado con `vm.Script`, ver §0.1). `T14` documenta que quedan bloqueadas permanentemente 4 pantallas por refresco eager sin guarda (Nueva vida/Agente de ahorro/Visual Detail/Reconciliation) — revisar si alguna puede desbloquearse cambiando el propio patrón de refresco antes de darlas por perdidas. **Primer incremento (24/09/2026, sesión 233): `#reconciliation` desbloqueada** — no era permanente: bastaba cambiar el patrón de refresco, no mover más código. Sus llamadores externos (cerrar/reabrir mes, también desde `#conciliar`) pasan por `refreshReconciliationView()` (`app.js`), que pinta solo con la pantalla abierta y su fichero cargado y, si no, conserva los dos efectos de estado que tenía (recalcular/persistir el libro conciliado y reevaluar la barrera de publicación). La pantalla pasa a `views/reconciliation.js`, verificada en navegador idéntica texto a texto a `main`. **Techo de `app.js`** (`tests/arq4-techo-app-js.test.cjs`, pedido por el hogar): 38.600 líneas / 2.000.000 bytes, con trinquete — si `app.js` baja más de 300 líneas, el mismo PR baja el techo. Motivo: `app.js` había vuelto a crecer +1.441 líneas desde la pausa de `T14`, anulando buena parte de su recorte. **Segundo incremento (24/09/2026, sesión 236): `#new-life-simulation` desbloqueada** — tampoco era permanente, y aquí el bloqueo documentado ni siquiera se sostenía: los tres llamadores sin guarda (GOB20 declarar/quitar, GOB12 aplicar a real) cuelgan de controles que viven *dentro* de la propia sección, no en Plan. Llevan ahora la guarda `viewChunkLoaded("new-life-simulation")` por si algún día se cablean desde fuera; la pantalla (12 funciones) pasa a `views/new-life-simulation.js`, verificada en navegador idéntica a `main` al entrar, tras declarar y tras quitar una caída de ingreso. El Laboratorio E13 y los paneles GOB20/GOB12 se quedan en `app.js` (sistema propio, 21 ficheros de tests los extraen de ahí). `app.js` 38.467 → 38.105; techo bajado a 38.235. **Diagnóstico de las dos que quedan**: *Agente de ahorro* — bloqueo real pero resoluble con el mismo patrón: `applyAgentRouteSimulation()`, disparable desde Hoy (centro de acciones unificado) y desde cuatro pantallas más, llama a `renderSavingsAgent()` en sus dos salidas sin cambios; el coste está en separar sus renderizadores del motor del agente, que comparten media docena de pantallas. *Visual Detail* — la más difícil: 11 llamadores, todos editores de su propia rejilla con cambios en borrador (`discardVisualChanges`, también desde la barra de impacto de Plan); requiere auditar el estado de borrador antes de moverla. Orden recomendado: Agente de ahorro → Visual Detail. **Tercer incremento (24/09/2026, sesión 237, opción «A» elegida por el hogar): «Agente de ahorro» desbloqueada, y su núcleo de cálculo a un módulo propio.** Las cuatro llamadas a `renderSavingsAgent()` desde `app.js` (entre ellas las dos de `applyAgentRouteSimulation()`, disparable desde Hoy) llevan la guarda `viewChunkLoaded("savings-agent")`; la pantalla (18 funciones, 749 líneas) pasa a `views/savings-agent.js`. El motor del agente se reparte según sea puro o no: la regla mensual de traspaso/rescate CaixaBank↔Mediolanum pasa a `canonical-savings-agent.js` (UMD, con 7 tests, uno de ellos contra la implementación anterior en 500 simulaciones aleatorias: idéntico al céntimo); el optimizador de deuda se queda en `app.js` porque vuelve a ejecutar la simulación completa del hogar por cada candidato. Verificado en navegador idéntico a `main` en seis pantallas (Agente, Hoy, Ejecutivo, Asesor virtual, Plan de deuda, Nueva vida). `app.js` 38.105 → 37.306; techo bajado a 37.436. **Queda solo Visual Detail**, la de más riesgo (edición con cambios en borrador) — recomendación: no moverla mientras no haya un motivo más allá del recuento de líneas. Detalle en los cierres de sesión 233, 236 y 237 de `PROJECT_STATE.md`. **Pausa (sesión 238)**: el hogar decide «dejar Visual Detail donde está» — la única pantalla que queda, la de más riesgo (cambios en borrador) y sin ningún efecto visible para el hogar; el techo de `app.js` ya impide que vuelva a crecer. Se reanuda solo si el hogar lo pide o si Visual Detail tiene que tocarse por otro motivo. |
| `ARQ-5` | ~~Caché offline completa: que todo lo que carga la app esté en la caché del Service Worker~~ — **cerrada el 24 de septiembre de 2026 (sesión 238)** | S | Muy alto | Sin fila previa: la encontró la sesión 238 al revisar la caché offline a petición del hogar. `SHELL_URLS` (`service-worker.js`) se mantenía a mano y nada la comparaba con lo que la app carga: 30 de los 65 `canonical-*.js` que `index.html` carga al arrancar (Oleadas 3 y 4, `PER-1`, fiscalidad, apalancamiento, seguros...) y `views/estado-semana.js` nunca entraron en ella, el más antiguo con sello del 9 de agosto. Sin red la app abría igual y sin un solo error de JavaScript, pero 21 de 59 pantallas pintaban de menos (Hoy sin salud financiera ni fecha de libre de deuda, Estado de la semana casi vacía, Previsión sin resumen por periodo, Inversión/Seguros/Patrimonio/Deuda sin sus avisos), con A0-4 marcada «Verificado». Corregido (74 → 105 recursos en caché; 107 con los dos ficheros del Agente de ahorro de la sesión 237, que sí entraron) y verificado en navegador: sin red, las 59 pantallas pintan lo mismo que con red. `tests/arq5-cache-offline-completa.test.cjs` (4 pruebas) exige que todo recurso local de `index.html` y toda vista de `VIEW_CHUNKS` esté en la caché, y que toda entrada de la caché exista y se publique (un solo 404 tumbaría la instalación entera); única exclusión, con motivo: `supabase-config.js` (A0-4, sin credenciales en caché). Detalle en el cierre de sesión 238 de `PROJECT_STATE.md`. |

### 2.3 Navegación y arquitectura de información (`NAV`)

| ID | Tarea | Esfuerzo | Beneficio | Nota |
|---|---|---|---|---|
| `NAV-1` | ~~Subcabeceras visibles para el grupo «Analizar» del menú avanzado (24 enlaces: Deuda/Inversión/Seguros/Fiscal/Patrimonio)~~ — **cerrada el 23 de septiembre de 2026 (sesión 228)** | S | Medio | De las cinco categorías, Seguros/Fiscal/Patrimonio ya tenían subcabecera visible (`OPT-25`); Deuda (4 enlaces) e Inversión (5 enlaces) vivían sin la suya, agrupadas de facto bajo «Decidir». Dos `<p class="advanced-nav-label" data-e17-nav-label="...">` nuevas, mismo patrón ya existente — sin CSS ni JS ni motor nuevo, sin tocar el fieldset de preferencias. Detalle completo en el cierre de sesión 228 de `PROJECT_STATE.md`. |
| `NAV-2` | ~~Buscador Cmd/Ctrl+K con resultados ponderados por frecuencia de uso real~~ — **cerrada el 23 de septiembre de 2026 (sesión 226)** | M | Alto | El buscador ya existía (`e17-experience.js`, `T2` cerrada) solo con coincidencia de texto, sin orden real — un `filter` puro sobre `TASKS` en su orden de declaración. `findTasks(query, normalize, getUsageWeight)` gana un tercer parámetro opcional (misma inyección que ya usaba `normalize`), que ordena los resultados filtrados por peso descendente con el orden original de `TASKS` como desempate estable — sin peso (valor por defecto), el comportamiento no cambia. `app.js` lo alimenta con `e17SearchUsageWeight`, que lee `viewVisitSummary`/`ARQ-0` y resuelve las cuatro claves heredadas de `REGISTRAR_LEGACY_HASH_TABS` al contador de "registrar", la pantalla donde de verdad aterrizan. Sin motor nuevo, sin UI nueva. Detalle completo en el cierre de sesión 226 de `PROJECT_STATE.md`. |
...
| `NAV-4` | ~~Breadcrumbs o indicador de ubicación en las pestañas de segundo nivel (5 de Inversión, 4 de Deuda)~~ — **cerrada el 23 de septiembre de 2026 (sesión 230)** | S | Bajo | Inversión ya seguía "Inversión · <Pestaña>" en su eyebrow; Deuda decía "Decidir · <descripción libre>" (vestigial desde que `NAV-1` la sacó de ese grupo) sin repetir "Deuda" ni la etiqueta exacta de la pestaña activa. Sin componente nuevo: las 4 entradas de `viewTitles` y sus párrafos gemelos en `index.html` pasan a "Deuda · <Pestaña>", mismo patrón que Inversión — el criterio real de `T17` (repetir en texto un dato que ya existe, en el punto de uso). Detalle completo en el cierre de sesión 230 de `PROJECT_STATE.md`. |
...
| ID | Tarea | Esfuerzo | Beneficio | Nota |
|---|---|---|---|---|
| `FLU-1` | ~~Test automático que fije la regla de 4 bloques de Home (`OPT-8`) como invariante, no solo como disciplina~~ — **cerrada el 23 de septiembre de 2026 (sesión 229)** | S | Medio | `tests/opt8-jerarquia-visual-hoy.test.cjs` ya existía pero solo comprobaba QUÉ cuatro bloques hay (por `id`), no CUÁNTOS — pasaría igual con una quinta tarjeta añadida sin querer. `tests/flu1-invariante-4-bloques-home.test.cjs` (nuevo, sin tocar producción) cuenta `.home-panel` + `.home-kpi-grid` dentro de `.home-primary-section` y falla si supera 4. Recuento real de hoy: 3 artículos + 1 rejilla. Detalle completo en el cierre de sesión 229 de `PROJECT_STATE.md`. |
| `FLU-2` | ~~Atajo de un clic «registrar gasto nuevo»...~~ — **cerrada el 23 de septiembre de 2026 (sesión 225)** | S→M/L | Alto | La app separaba en dos pantallas crear una partida nueva (Planificación de partidas, solo previsto) de marcarla como real ya ocurrida (Registrar › Reales del mes, solo edita partidas existentes). El hogar eligió explícitamente el modal de un solo paso (M/L) sobre la navegación directa más barata (S) — el único que cumple "un clic, gasto registrado" con el importe real incluido. Botón `+ Registrar gasto` en la cabecera de Home (fuera de `.home-primary-section`, no cuenta para la regla de 4 bloques de `OPT-8`/`FLU-1`) abre un `<dialog>` de tres campos (concepto, bloque, importe — sin selector de mes, siempre el mismo que Registrar › Reales del mes resolvería por defecto) que escribe a la vez en `customPlanningRows` y `expenseActuals`, los mismos dos almacenes que ya usaban `handlePartidasAddRow`/`handleRegistrarActualsChange` por separado, sin motor nuevo. Detalle completo en el cierre de sesión 225 de `PROJECT_STATE.md`. |
...
| ID | Tarea | Esfuerzo | Beneficio | Nota |
|---|---|---|---|---|
| `FIN-1` | ~~Guardarraíl único de liquidez mínima compartido entre cancelación de deuda (`DEB15`), apalancamiento y rebalanceo de inversión~~ — **cerrada el 23 de septiembre de 2026 (sesión 227)** | M | Alto | La auditoría encontró que `DEB15`/`AP3`/`AP4`/`AP6`/`INV7` ya compartían exactamente el mismo umbral (`cushionFloor(lastSimulation, cuadroMandosReserve()).value`) — sin divergencia de cifras entre esos cinco puntos. El hueco real era otro: el rebalanceo de inversión (`IV6`) no consultaba ningún colchón, y podía sugerir vender liquidez inmediata para comprar un tipo bloqueado (plan de pensiones) sin avisar de que eso rompe la cobertura que `INV7` (pestaña de al lado) da por garantizada hoy. `rebalanceLiquidityGuardrail()` (`canonical-portfolio.js`) proyecta la composición que dejaría la sugerencia de `IV6` y reutiliza `liquidityLadder()` (el mismo motor de `INV7`) para avisar solo cuando el colchón pasaría de cubierto a no cubierto — sin motor nuevo, sin bloquear nada. Detalle completo en el cierre de sesión 227 de `PROJECT_STATE.md`. |
| `FIN-2` | ~~Test de integración fiscal cruzada: IRPF + dividendos + plusvalías + compensación de pérdidas en el mismo ejercicio~~ — **cerrada el 24 de septiembre de 2026 (sesión 232)** | M | Medio | `tests/fin2-integracion-fiscal-cruzada.test.cjs` recorre un ejercicio completo (FIFO de FC1 → compensación FC3 con pérdidas arrastradas → venta que se valora en FC5 → dividendo FC4 → rescate de pensión en la base general, FCX1/I5/A15-2) y comprueba los puntos de cruce, no cada motor por separado. **Destapó un bug real en producción**: FC5 (`optimizePartialSale`) recortaba a 0 una base negativa, así que la cadena FC3 → FC5 que la propia pantalla sugiere cobraba impuesto sobre plusvalía que en realidad absorbían las pérdidas del año o las arrastradas todavía disponibles (en el caso de la prueba, 950 € estimados frente a 779 € reales). Corregido en el motor (base negativa admitida, nuevo `absorbedByLosses`), en FC3 (nuevo `marginalSaleBase`: neto del año menos todas las pérdidas arrastradas aún aplicables — `taxableNet` no servía, se queda en 0 y esconde las que siguen disponibles), en la UI (FC3 dice qué cifra llevar a FC5, el campo de FC5 ya admite negativos) y en INV18, que compartía el mismo recorte. Detalle completo en el cierre de sesión 232 de `PROJECT_STATE.md`. |
...
| `PROC-2` | ~~Memorizar la checklist mensual de Nielsen (`OPT-21`) como paso explícito del flujo de cierre de sesión cuando corresponda el mes~~ — **cerrada el 24 de septiembre de 2026 (sesión 234)** | S | Medio | La mitad de la tarea ya estaba hecha: `docs/OPT21_CHECKLIST_NIELSEN.md` ya reflejaba `T15`-`T19` como cerradas (tercera vez, tras la Mejora #9 y `ARQ-0`, que esta auditoría pide algo ya resuelto). Lo que faltaba de verdad: la skill `finanzas-casa-workflow` (`.claude/skills/finanzas-casa-workflow/SKILL.md`) gana un paso en Modo Inicio (si han pasado 30 días o más desde la última revisión registrada, se dice y entra en la propuesta de plan) y otro en Modo Cierre (si sigue vencida y no se hizo, queda anotado en la entrada de `PROJECT_STATE.md`). `tests/proc2-nielsen-en-flujo-de-sesion.test.cjs` protege ese paso sin depender del calendario (nunca pone el CI en rojo solo porque pase el tiempo). Próxima revisión: vence el 16 de octubre de 2026. |
...
| ~~`T9`~~ | ~~Migración mobile-first, siguiente incremento auditado~~ — **redefinida y cerrada el 24 de septiembre de 2026 (sesión 239)**: arreglado el contenido cortado fuera de la pantalla (13/59 pantallas en móvil, 5-6 en escritorio) y vigilado en el CI a 360/768/1280px | — | 2.0 §4 |
...
| 3 — este semestre | ~~`FLU-2`~~ cerrada (23/09/2026, sesión 225); ~~`NAV-2`~~ cerrada (23/09/2026, sesión 226); ~~`FIN-1`~~ cerrada (23/09/2026, sesión 227); ~~`NAV-1`~~ cerrada (23/09/2026, sesión 228); ~~`FLU-1`~~ cerrada (23/09/2026, sesión 229); ~~`NAV-4`~~ cerrada (23/09/2026, sesión 230); ~~`ARQ-3`~~ cerrada (23/09/2026, sesión 231); ~~`ARQ-5`~~ cerrada (24/09/2026, sesión 238, caché offline completa — sin fila previa, entró por delante de todo por ser una función prometida rota); `ARQ-4` **en pausa** por decisión del hogar (sesión 238; tres incrementos publicados en las sesiones 233, 236 y 237 — solo queda Visual Detail); ~~`T9`~~ cerrada (24/09/2026, sesión 239, redefinida: contenido cortado en móvil y escritorio, con comprobación en el CI) — **horizonte 3 completo** | Impacto directo en uso diario y deuda técnica, sin requerir motores nuevos ni depender de telemetría |
| 4 — coste medio, sin urgencia | `NAV-3`, `NAV-5`, `FLU-3`, ~~`FIN-2`~~ cerrada (24/09/2026, sesión 232, adelantada al horizonte 3 a petición del hogar), `PROC-1`, ~~`PROC-2`~~ cerrada (24/09/2026, sesión 234); revisar `D1`/`D6`/`I3`/`I6`/`I13` (heredadas) según sus propias condiciones | Sin bloqueo externo, pero de menor urgencia que el horizonte 3 |
