...
| E3 | A0-4, A0-5 y A0-9 | Apertura offline, recuperación guiada y copia de emergencia | Verificado | Apertura offline, recuperación guiada y copia de emergencia aceptadas. **A0-4 estuvo degradada sin saberlo desde agosto hasta el 24/09/2026** (30 motores y una vista fuera de la caché offline); restablecida y blindada con prueba en `ARQ-5` (sesión 238). **A0-9 también estaba rota, por partida doble** (`ARQ-6`, sesión 241): desde el 15/08/2026 (R-10/R-11) el panel de copia vivía en `#data-entry`, que redirige a Registrar y nunca se muestra, así que no había ningún botón para descargar la copia ni para restaurarla; y la copia, descargada a mano, perdía 14 almacenes propios (historia de cada cierre, escenarios guardados, datos escritos a mano). Panel movido a Ajustes › Datos y exportación, almacenes incluidos y viaje completo vigilado en el CI |
...
| E18 | A13-1 a A13-6 | Arquitectura, rendimiento, telemetría, migraciones, pruebas y manual | Verificado | Fixtures históricos, métricas locales, guía contextual y doce capturas comparables aceptadas. **Las capturas no las vuelve a comprobar nada**: se hicieron en macOS y no pueden compararse en el CI (Linux). Desde el 24/09/2026 (`ARQ-6`, sesión 240) sus seis pantallas se comprueban en el CI por comportamiento (QA-1: abren, pintan, sin errores ni bloqueos largos) y por recorte (`T9`). Al añadirlas salió que abrir Control de deuda bloqueaba la página ~12 s; corregido |
...
| --- | --- | --- | --- |
| La caché offline requiere versionado disciplinado | Tras publicar E5 una pestaña mostró todavía el aviso de formato remoto antiguo; el shell abre sin servidor tras una primera visita | Una caché obsoleta puede retrasar recursos nuevos o mostrar una migración ya atendida | Mantener recursos versionados, recargar el shell antes de migrar y comprobar el arranque offline en cada cambio. Desde `ARQ-5` (24/09/2026) la completitud de la caché la vigila una prueba, no la disciplina: la comprobación manual no bastó y la caché llevaba semanas incompleta |
...
| A0-4 | Apertura offline del shell | Verificado | Alta | Tras una primera visita, la app abre sin red con recursos versionados; no se almacenan credenciales ni respuestas privadas en caché compartida. **Regresión silenciosa detectada el 24/09/2026 (`ARQ-5`, sesión 238)**: 30 de 65 `canonical-*.js` y `views/estado-semana.js` no estaban en la caché offline (el más antiguo lleva sello del 9 de agosto); sin red la app abría sin errores pero 21 de 59 pantallas pintaban de menos. Corregido, verificado en navegador pantalla a pantalla, y `tests/arq5-cache-offline-completa.test.cjs` compara ya la caché con `index.html`, `VIEW_CHUNKS` y el sitio publicado |
