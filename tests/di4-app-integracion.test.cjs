...
test("DI4: renderInversionApalancamiento sincroniza y rellena la nota del aval (OPT-24: ya no es un ajuste, es una herramienta)", () => {
  const start = inversionSource.indexOf("function renderInversionApalancamiento(");
  const end = inversionSource.indexOf("\n}", start);
  const body = inversionSource.slice(start, end);
