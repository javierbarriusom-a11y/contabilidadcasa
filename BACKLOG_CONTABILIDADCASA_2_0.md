...
200-205). `T14` (Horizonte 3) arrancó en la sesión 206 (`views/escenarios.js`) y suma cinco
...
| ⏳ `T14` | Reducir el monolito técnico (`app.js` 2,1MB) | L | Alto | **Seis incrementos construidos (sesiones 206-207), sigue pendiente el resto.** El hogar eligió módulos ES entre las dos rutas planteadas, pero el repositorio ya tiene dos patrones de modularización propios y probados (63 `canonical-*.js` con factory UMD para lógica pura; `views/*.js` con carga diferida para código de vista con estado compartido, ya usado por Deuda/Inversión) — introducir `import`/`export` habría sido un tercer sistema incompatible sin resolver el problema real (los tests cargan `app.js` con `vm.Script`, que no soporta módulos ES). El hogar confirmó seguir con el patrón ya existente. Sesión 206: `views/escenarios.js` (hub Escenario, 4 pantallas, 65 funciones). Sesión 207: `views/deuda.js` (comparador de las 8 estrategias de deuda, 33 funciones) — auditoría equivalente detectó y corrigió, de paso, dos `ReferenceError` ya publicados en el primer incremento; `views/debt-liquidation-plan.js` (plan de deuda óptimo heredado, pantalla `legacy`, 20 funciones); `views/virtual-advisor.js` (asesor virtual, 23 funciones), `views/executive-advisor.js` (ejecutivo, 9 funciones) y `views/new-life-definitive.js` (nueva vida definitiva, 32 funciones) — tres porciones self-contained de un cluster mayor (Ejecutivo/Nueva vida/Nueva vida definitiva/Asesor virtual/Agente de ahorro). "Nueva vida" (simulación) no se puede mover sin cambiar comportamiento: varios manejadores de Plan/Ajustes la refrescan sin guarda de pantalla activa. El "motor" compartido (catálogo de tipos, `runEscenarioMotor`, `escenarioMotorSummaryFor`, `missingScenarioDependencies`, la Central de Acciones Unificada de Hoy...) y el "agente de ahorro" (`buildSavingsAgentPlan` y familia) se quedan en `app.js` porque los reutilizan varias pantallas eager/lazy independientes; el render propio del agente de ahorro (`renderSavingsAgent`) todavía no se ha auditado para el mismo patrón. `app.js`: 41.596 → 37.867 líneas (-9,0% acumulado). Detalle completo, incluida la verificación en navegador real, en los cierres de sesión 206 y 207 de `PROJECT_STATE.md`. |
...
**seis incrementos construidos, sesiones 206-207**, tras que el hogar decidiera seguir con el patrón
