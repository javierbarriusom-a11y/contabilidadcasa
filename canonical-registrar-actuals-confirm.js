(function attachCanonicalRegistrarActualsConfirm(root, factory) {
  ...
  /** @param {{escapeHtml: (value: unknown) => string, disabled?: boolean}} options */
  function bulkButtonHtml(pendingCount, blockFilter, options) {
    const { escapeHtml, disabled = false } = options || {};
    if (!pendingCount) return "";
    const scope = blockFilter === "todos" ? "todos los pendientes" : `los pendientes de ${blockFilter}`;
    const plural = pendingCount === 1 ? "" : "s";
    return `<button type="button" class="e19-btn e19-btn-secondary" data-registrar-actuals-confirm-pending${disabled ? " disabled" : ""}>Confirmar ${escapeHtml(scope)} con su previsto (${pendingCount} partida${plural})</button>`;
  }

  // Un único punto de entrada para el clic delegado del panel entero: decide, a partir del elemento
  // pulsado, qué partidas tocaría confirmar (una, las de un bloque, o todas) — o null si el clic no
  // era de este botón, para que app.js siga con su propia lógica de navegación.
  function entriesForClick(target, { allEntries, blockFilter }) {
    ...
    if (target?.closest?.("[data-registrar-actuals-confirm-pending]")) {
      return (allEntries || []).filter((entry) => (blockFilter === "todos" || entry.sectionName === blockFilter) && !entry.hasActual);
    }
    return null;
  }
