...
| 2 — este trimestre | ~~`PER-1`→`PER-2`→`PER-3`→`PER-4`~~ cerradas (22/09/2026, sesión 221; `PER-4` con alcance recortado, ver su fila en §2.1); `I5` (heredada, sin bloqueo); `ARQ-1`, `ARQ-2` | El bloque completo del modelo de periodo queda cerrado esta sesión, con una excepción deliberada: la salud financiera se queda sin selector de periodo, decisión explícita del hogar (no tenía ningún concepto de periodo previo del que partir). `I5` ya era la siguiente tarea acordada con el hogar (2.0 §6) — sigue siendo lo próximo del horizonte 2 |
