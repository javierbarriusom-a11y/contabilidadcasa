...
| 17 | ⏳ INV10 | Vender activo vs. pedir prestado contra él | Inversión / Apalancamiento | L | Alto | **Decidido: comparador genérico** para cualquier meta (no acotado a una sola). Pendiente de construir — compone `opportunityCost` (IV5) + `lombardCreditCapacity`/`netDebtCostAfterTax` (APX2/APX1), mismo patrón que ya usa `deb1RecomputeTrackedAssessment`. |
| 18 | ✅ LEV5 | Colchón de garantía dinámico según volatilidad declarada | Apalancamiento | M-L | Medio | **Decidido: banda declarada a mano.** Hecho (sesión 159). |
| 19 | ✅ LEV6 | Plan de desapalancamiento con prioridad | Apalancamiento | L | Medio | **Decidido: sí, sustituir el tercer criterio por "misma clase ya sobreexpuesta".** Hecho (sesión 159). |
| 20 | ✅ LEV7 | Seguro de cola frente a margin call | Apalancamiento | L | Medio | **Decidido: usar la banda P10 ya calibrada de `ESX1`**, sin calibración de cola aparte. Hecho (sesión 159). |
...
| 23 | ⏳ PVC5 | Recalibración trimestral del triángulo Monte Carlo | Previsión viva | M-L | Medio | **Decidido: ventana de 8 trimestres, con confirmación del hogar antes de aplicar** (nunca automático). Pendiente de construir. |
| 24 | ⏳ PVC10 | Previsión ponderada por eventos inciertos | Previsión viva / Escenarios | M-L | Medio | **Decidido: las dos ubicaciones** — línea permanente en el forecast principal Y dentro del Laboratorio de escenarios. Pendiente de construir. |
