...
**Actualizado el 5 de septiembre de 2026 (sesiones 137-150): los Bloques 1 a 4 de esa cola ya están
cerrados y el Bloque 5 va por la mitad (47/51 tareas).** Bloque 1 (entrada de datos, 11 tareas) fue
...
147, `APX3` sesión 148) también 100% resuelto. Del Bloque 5 (5 tareas, esfuerzo L/M-L) están hechas
`PVX5` (sesión 149) y `ESX3` (sesión 150, que además corrigió un bug real preexistente en
`simulate()` de `canonical-e13-scenarios.js`). **Quedan 3 tareas sin empezar: `ESX1`/`IVX3`/`FCX2`
del Bloque 5 (las tres con preguntas de alcance abiertas planteadas al usuario, sin resolver
todavía), y `RGX3` en el Bloque 6 condicionada a que `A5-1` esté activo en producción.**
...
| --- | --- | --- | --- |
| **`BACKLOG_ULTIMATE_SEPTIEMBRE_OLEADA_2.md`** | 🟡 **Vigente — Bloques 1-4 cerrados, Bloque 5 a la mitad (47/51), quedan 4 tareas** | 51 tareas nuevas (previsión viva, escenarios, entrada de datos, inversión, apalancamiento, deuda según liquidez, copiloto, fiscalidad, patrimonio, continuidad e IA, multidispositivo) en 6 bloques; el Bloque 1 (entrada de datos) fue primero por decisión explícita | — |
