...
|---|---|---|
| 🔍 `VER-4` | ¿`fiscalAdjustedDebtPriority()` (`DEB5`) ya se invoca desde `surplusAllocationRule()` (`DLX2`) cuando el veredicto es "amortizar", o vive como función aislada sin cablear a la asignación automática del excedente? | Alcance final de `DEB10` (antes `DE-2`): si ya está cableada, `DEB10` se reduce a un aviso visible; si no, incluye el cableado. |
| 🔍 `VER-5` | ¿El asistente (`canonical-e9-assistant.js`) ya cita la función `canonical-*.js` concreta que sustenta cada respuesta (patrón `recommendation-citation`), o construye una narrativa aparte de la evidencia? | Alcance final de `GOB17` (antes `O-9`): si ya cita, la tarea se retira; si no, se construye completa. |
| 🔍 `VER-6` | ¿`categoryDriftWindows()` (`PVC4`) comparte alguna fuente de datos con `_detectMonthlySeasonality()` (`canonical-budget-forecast-category.js`), o son dos cálculos totalmente independientes sobre los mismos meses cerrados? | Alcance final de `PVC12` (antes `PA-2`): decide si consolidar es un cambio de cableado o exige reconciliar dos modelos matemáticos distintos. |
...
|---|---|---|---|---|---|---|
| 1 | ⏳ `LEV9` | Comparador cruzado de instrumentos de apalancamiento (Lombard / hipoteca / línea de crédito) para una misma necesidad de capital | `AP-1` | L | Crítico | Sin precedente en la Oleada 3: `LEV4` solo compara Lombard entre entidades, `INV10` solo compara vender vs. pedir prestado contra Lombard. Cruza `lombardCreditCapacity` (`APX2`), `evaluateMortgageRateScenarios` y `evaluateEmergencyCreditLine`, mostrando además qué guardarraíl aplica a cada uno (`AP4` no cubre Lombard, `LEV1` sí incluye el techo pero no las condiciones mínimas de Lombard). |
| 2 | ⏳ `DEB9` | Síntesis única "cancelar vs. mantener deuda", con las cuatro piezas visibles | `DE-1` | L | Alto | `DEB1` (Oleada 3) solo vigila si el veredicto binario de `AP1` cambió de sentido; no cruza `netDebtCostAfterTax` (`APX1`) + `waitingOptionValue` (`DEB3`) + `dimensionOptimalPrepayment` (`DEB2`) + liquidez real (`INV7`) en una sola recomendación explicada. |
...
| 4 | ⚠️ `PVC12` | Consolidar los mecanismos de estacionalidad/deriva por categoría | `PA-2` | M (pendiente de `VER-6`) | Alto | Alcance reducido: con `PVC4` (Oleada 3) ya hay un tercer mecanismo de deriva por categoría además del `learnFromHistory` agregado y `_detectMonthlySeasonality`. La tarea deja de ser "construir el filtro anti-ruido por categoría" y pasa a ser "auditar y consolidar tres motores que hoy no se hablan entre sí" — el alcance exacto lo fija `VER-6`. |
...
| Orden | ID | Tarea | Origen | Esfuerzo | Beneficio | Nota |
|---|---|---|---|---|---|---|
| 29 | ⚠️ `DEB10` | Al amortizar, priorizar automáticamente qué deuda concreta primero | `DE-2` | S-M (pendiente de `VER-4`) | Alto | Alcance reducido: `DEB5` (Oleada 3, `fiscalAdjustedDebtPriority()`) ya ordena todas las deudas por TAE tras deducción fiscal. El alcance exacto (solo cablear a `DLX2`, o falta también algo del cálculo) lo fija `VER-4`. |
...
| 43 | 🔍→⏳ `GOB17` | El asistente cita siempre la función `canonical-*.js` real que sustenta su respuesta | `O-9` | M (pendiente de `VER-5`) | Alto | Si `VER-5` confirma que ya cita, esta tarea se retira; si no, se construye extendiendo el patrón `recommendation-citation` ya usado en otras áreas. |
