...
test("saveA18Rule · sin categoría seleccionada, guarda la regla por defecto", () => {
  ...
  const ctx = sandboxWith(["householdSplitSettings", "saveHouseholdSplitSettings", "saveA18Rule", "renderA18RuleList", "a18RuleLabel"], {
    scenarioSettings,
...
test("removeA18Rule · quita solo la categoría indicada", () => {
  ...
  const ctx = sandboxWith(["householdSplitSettings", "saveHouseholdSplitSettings", "removeA18Rule", "renderA18RuleList", "a18RuleLabel"], {
    scenarioSettings,
    qs: () => ({ innerHTML: "" }),
  });
