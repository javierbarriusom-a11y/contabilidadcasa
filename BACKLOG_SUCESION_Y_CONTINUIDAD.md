...
| ✅ `LPX6` | Beneficiario declarado por póliza de vida | S | Medio | Hecho (sesión 193). El inventario real de pólizas (`SP1`) es `insurancePolicies()` en `app.js`, no `canonical-life-coverage.js` (que es `SP2`) — diagnóstico corregido en `PROJECT_STATE.md`. Campos opcionales `isLife`/`beneficiary` por póliza; `LPX3` sustituye su casilla manual global "beneficiarios" por una comprobación real (al menos una póliza de vida con beneficiario declarado) — mismo tipo de cierre de bucle que `PVC13` hizo con `confidenceBands()`. |
