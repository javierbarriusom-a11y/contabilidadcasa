...
test("AP1: la lista de deudas se renderiza al abrir Deuda › Apalancamiento (OPT-24: ya no es un ajuste, es una herramienta), justo después de la alerta de AP6", () => {
  assert.match(inversionSource, /renderAp6Alert\(\);\s*\n\s*renderAp1DebtOptions\(\);/);
});
...
test("DLX1: el guardarraíl se antepone a la lectura amortizar/invertir, no la sustituye", () => {
  ...
  // Bloque 6) añadió la tarjeta de reducir cuota vs. plazo sobre el importe dimensionado; I13
  // (BACKLOG_CONTABILIDADCASA_2_0.md §2) añadió el cálculo del objetivo de ahorro antes del
  // pintado — la ventana crece de 2200 a 3100 a 3900 a 4600.
