...
|---|---|---|---|---|
| ⏳ `P1` | Panel «qué cambió desde la última vez» en Hoy | S | Alto | Reutiliza `causalTreeForMonth`/`previsionChangeOneLiner` (`PVX5`) y `diffAssumptionSnapshots` (`PVC6`), hoy solo visibles en Ajustes. |
...
## 4. Transversales y nuevas (`T1`-`T14`)
...
| ID | Tarea | Esfuerzo | Beneficio | Nota |
|---|---|---|---|---|
| ⏳ `T1` | Ejecutar por fin la auditoría Nielsen real sobre Hoy/Registrar/Plan | S | Alto | **Máxima prioridad de todo el documento.** El marco existe desde el 29/08 (`docs/OPT21_CHECKLIST_NIELSEN.md`); nunca se produjo un hallazgo real. Debería ir antes que cualquier otra tarea de este documento en esas 3 pantallas. |
...
No se recomienda abordar las 47 a la vez — sería repetir el mismo error de fondo que ya produjo
37 pantallas. Tres horizontes:

**Horizonte 1 — ya (bajo esfuerzo, alto impacto):** `T1`, `T2`, `D4`, `I11`, `P1`, `P5`, `P6`.
