...
test("IV3: renderIv1ScheduledContributionOptions existe y se llama junto al resto de renders de cartera", () => {
  ...
  // IVX6 añadió el campo goalId, IVX4 (misma oleada) el campo feePct, INV1 (Oleada 3 Bloque 4) el
  // campo assetClass, e INV14/INV15/INV20 (Oleada 4, Bloque 4) más campos, cada uno con su
  // comentario, antes de esta llamada — la ventana crece a 4400, la comprobación sigue siendo la
  // misma.
  const saveBlock = appSource.slice(appSource.indexOf("function saveIv1Position("), appSource.indexOf("function saveIv1Position(") + 4400);
